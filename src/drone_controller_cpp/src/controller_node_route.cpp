...
#include <sstream>           // Обработка строк
...
// Путь к файлу маршрута
  std::string route_file_ = "src/drone_controller_cpp/src/route.csv";
...
// Начальное положение точки взлёта с координатами аэропорта Канберры
  ...
  float alt = 5.0;             // Высота над крышей дома
  double max_distance = 1000.0; // Предельное удаление
...
// 3. Считываем маршрут из файла
    ...
    if(!file.is_open()) {
      std::cerr << "Ошибка: не удалось открыть файл маршрута: " << route_file_ << std::endl;
      RCLCPP_FATAL(this->get_logger(), "Не удалось открыть файл маршрута: %s", route_file_.c_str());
...
// Выводим телеметрию
    ...
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 4000,
      "Секунда: %02d, широта: %.7f, долгота: %.7f, высота: %.1f, заряд: %2.0f",
      elapsed_sec, current_lat_, current_lon_, current_alt_, current_charge_);
...
// Конечный автомат
    switch (current_state_) {
      ...
      case FlightState::FLY_TO_POINT: {
        if (current_charge_ < 20) {
          current_state_ = FlightState::RETURN_TO_BASE;
          break;
        }
        double distance_from_base = haversine(current_lat_, current_lon_, home_lat, home_lon);
        if (distance_from_base > max_distance) {
          current_state_ = FlightState::RETURN_TO_BASE;
          break;
        }
      ...
      case FlightState::WAIT_RTL_RESPONSE:
        if (current_charge_ < 5) {
          current_state_ = FlightState::SEND_LANDING;
          break;
        }
      ...
      case FlightState::ERROR:
        RCLCPP_FATAL(this->get_logger(), "Завершение м ошибкой...");
        rclcpp::shutdown();
        break;          
    }
  }
...
// Проверка прибытия в точку
  void check_waypoint_arrival() {
    ...
    if (vertical_dist < 0.5 && horizontal_dist < 0.6) {
