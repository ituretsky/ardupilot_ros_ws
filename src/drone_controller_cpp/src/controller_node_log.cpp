...
// Файлы
  std::string route_file_ = "src/drone_controller_cpp/src/route.csv"; // маршрут
  std::string log_file_ = "src/drone_controller_cpp/src/treck.csv";   // журнал
  std::ofstream treck_file;                                           // поток журнала
...
// 5. Создаём журнальный файл, пишем заголовок
    treck_file.open(log_file_);
    treck_file << "секунда,широта,долгота,высота,заряд,режим,шаг\n";
  }
...
// Выводим телеметрию и пишем в лог
    ...
    if (treck_file.is_open()) treck_file << log_line << "\n";
